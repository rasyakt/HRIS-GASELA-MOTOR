        ...
        <CardContent className="p-4 space-y-4">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            >
              ...
              />
              <Button type="submit" variant="outline" size="sm" className="h-9 text-xs shrink-0">
          ...
          {warningsQuery.isLoading ? (
              ...
              <div className="grid grid-cols-1 gap-3 lg:hidden">
                {warningsQuery.data.items.map((sp) => {
                  ...
                  return (
                    <div
                      key={sp.id}
                      className="flex flex-col gap-2.5 rounded-xl border border-zinc-200 dark:border-zinc-800 bg-white dark:bg-zinc-900 p-3.5 shadow-2xs"
                    >
                      <div className="flex items-start justify-between gap-2">
                        <div>
                          <p className="font-semibold text-sm text-zinc-900 dark:text-white">
                            {sp.employee.fullName}
                          </p>
                          <p className="text-[11px] text-zinc-500 dark:text-zinc-400 font-mono">
                            NIK: {sp.employee.employeeNumber} · {sp.employee.department?.name || '—'}
                          </p>
                        </div>
                        <Badge className={`${SP_BADGE_STYLE[sp.level]} text-[10px] font-bold shrink-0`}>
                          {sp.level}
                        </Badge>
                      </div>

                      <div className="rounded-lg bg-zinc-50 dark:bg-zinc-850/60 p-2.5 text-xs">
                      ...
                      <div className="flex flex-wrap items-center justify-between gap-2 text-xs pt-2 border-t border-zinc-100 dark:border-zinc-800">
                        <div>
                          <p className="text-[10px] text-zinc-400 dark:text-zinc-500">Masa Berlaku</p>
                          <p className="font-medium text-zinc-700 dark:text-zinc-300">
                            {fmtDate(sp.issuedDate)} – {fmtDate(sp.effectiveUntil)}
                          </p>
                        </div>
                        <div>
                          {isExpired ? (
                            ...
                            <span className="inline-flex items-center rounded-full bg-red-100 dark:bg-red-950/60 px-2 py-0.5 text-[10px] font-semibold text-red-600 dark:text-red-400">
                              Aktif
                            </span>
                          )}
                        </div>
                          ...
                          <td className="p-3 text-right">
                            {isHR && (
                              <Button
                                ...
                                className="size-7 p-0 text-red-600 hover:text-red-800 dark:text-red-400 dark:hover:text-red-300"
                              >
            ...
            <div className="flex flex-col items-center justify-center py-12 text-center rounded-xl border border-dashed border-zinc-200 dark:border-zinc-800 p-12">
