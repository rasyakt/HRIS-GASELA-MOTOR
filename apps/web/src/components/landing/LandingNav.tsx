  ...
  return (
    <header
      className={`fixed top-0 inset-x-0 z-50 transition-all duration-300 ${
        scrolled
          ? 'bg-white/90 backdrop-blur-md border-b border-slate-200/80 py-3 shadow-xs dark:bg-zinc-950/90 dark:border-zinc-800/80'
          : 'bg-white/60 md:bg-transparent backdrop-blur-xs md:backdrop-blur-none border-b border-slate-200/50 md:border-transparent py-4 dark:bg-zinc-950/60 dark:md:bg-transparent dark:border-zinc-800/40 md:dark:border-transparent'
      }`}
      role="banner"
    >
      <nav
      ...
      >
        ...
        <a
          href={brandHref}
          className="group flex items-center gap-3.5 focus:outline-hidden focus-visible:ring-2 focus-visible:ring-amber-500 rounded-lg"
        ...
        >
          ...
          <div className="flex flex-col">
            <span className="text-slate-900 dark:text-white font-extrabold text-sm tracking-tight leading-tight">
              {nav.brandName}
            </span>
            <span className="text-[11px] font-semibold text-amber-700 dark:text-amber-400 tracking-wider uppercase">
              {nav.brandTagline}
            </span>
          </div>
        </a>
        ...
        <div className="md:hidden flex items-center gap-2">
          ...
          <button
            type="button"
            className="w-9 h-9 rounded-lg border border-slate-200 bg-white/90 text-slate-700 hover:text-slate-950 dark:border-zinc-800 dark:bg-zinc-900/90 dark:text-zinc-300 dark:hover:text-white transition-colors flex items-center justify-center shadow-xs"
      ...
      {/* Mobile Menu Drawer */}
      {mobileOpen && (
        <div className="md:hidden bg-white/95 backdrop-blur-xl border-t border-slate-200/80 px-6 pt-4 pb-6 shadow-xl dark:bg-zinc-950/95 dark:border-zinc-800/80">
            ...
            {links.map((link) => (
              <li key={link.label} className="pt-2 first:pt-0">
                <a
                  ...
                  className="flex items-center justify-between py-2.5 text-sm font-semibold text-slate-700 hover:text-slate-950 dark:text-zinc-300 dark:hover:text-white transition-colors"
                >
